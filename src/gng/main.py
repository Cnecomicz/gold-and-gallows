...
class Game:
    def __init__(self):
        ...
        self.DISPLAY_SURF = pygame.display.set_mode((gc.WINDOW_WIDTH, gc.WINDOW_HEIGHT))
        ...
        self.system_event_handler.register_event_handler(pygame.QUIT, self.quit_game)
        ...
        self.character_sheet_event_handler = cseh.CharacterSheetEventHandler(
            self.character_sheet_manager, None  # self.gameplay_state_machine_manager
        )
        self.dialogue_event_handler = deh.DialogueEventHandler(
            self.dialogue_manager, self.clock_manager
        )
        ...
        # Updaters -----------------------------------------------------
        self.manual_controls_updater = mcu.ManualControlsUpdater(self.manual_controls)
        self.clock_updater = cu.ClockUpdater(self.clock_manager)
        ...
        self.character_creator_artist = cca.CharacterCreatorArtist(
            self.character_creator_manager, self.player
        )
        self.debugging_artist = da.DebuggingArtist(
            self.debugging_manager, self.clock_manager
        )
        self.dialogue_artist = dia.DialogueArtist(self.dialogue_manager)
        self.character_sheet_artist = csa.CharacterSheetArtist(
            self.character_sheet_manager, self.player
        )
        ...
        self.character_creator_manager.system_event_handler = self.system_event_handler
        ...
        self.character_creator_manager.debugging_updater = self.debugging_updater
        self.character_creator_manager.debugging_artist = self.debugging_artist
        ...
        self.character_sheet_manager.system_event_handler = self.system_event_handler
        ...
        self.character_sheet_manager.debugging_updater = self.debugging_updater
        self.character_sheet_manager.debugging_artist = self.debugging_artist
        ...
        self.guy1.dt = g1dt.Guy1DialogueTree(self.gameplay_state_machine_manager)
    ...
    def handle_pygame_events(self):
        for pygame_event in pygame.event.get():
            for handler in self.list_of_active_x_manager.list_of_active_handlers:
                handler.handle_pygame_event(pygame_event)

    def update(self):
        for updater in self.list_of_active_x_manager.list_of_active_updaters:
            updater.update()

    def draw(self):
        self.DISPLAY_SURF.fill(gc.BGCOLOR)
        for artist in self.list_of_active_x_manager.list_of_active_artists:
